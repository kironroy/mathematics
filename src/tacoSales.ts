...
class TacoSalesCalculator {
  private readonly data: TacoData = {
    prices: {
      beef: 2,
      chicken: 2,
      steak: 4,
  ...
  private readonly salesData: Record<string, DailySales> = {
    Monday: {
      beef: 20,
      chicken: 22,
      steak: 40,
  ...
  calculateDailySales(day: string): number {
    const sales = this.salesData[day];
    return (
      sales.beef * this.data.prices.beef +
  ...
  calculateTotalSales(): number {
    return this.data.days.reduce(
      (total, day) => total + this.calculateDailySales(day),
      0
    );
  }
